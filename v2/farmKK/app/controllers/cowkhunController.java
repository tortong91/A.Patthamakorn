...
//            }else {
        ...
        if (picture != null) {
            ...
            int h = myForm.get().getPrice();
            String j = myForm.get().getSex();
            String a = myForm.get().getColor();
            Date z = myForm.get().getDate();
            int s = myForm.get().getAge();
            String g = myForm.get().getPicture();
            int l = myForm.get().getWeight();
            String i = myForm.get().getEat_recip();

            data1 = new Cows(typeid, data2, k, h, j, a, z, s, g, l,i);
    data1.setEat_recip("ยังไม่จัดสูตร");
            Cows.add(data1);
